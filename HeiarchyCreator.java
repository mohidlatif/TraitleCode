...
public class HeiarchyCreator {
    ...
    public HeiarchyCreator(){
        ...
        heiarchy.addGeneration(3, new Generation());
    }
  ...
  public void createHeiarchy(Couple c, int n, Gen gen) {
        switch (n) {
            case 0 -> heiarchy.getGeneration(0).addCouple(c);
            case 1 -> heiarchy.getGeneration(1).addCouple(c);
            case 2 -> heiarchy.getGeneration(2).addCouple(c);
            case 3 -> {
                Person child = (c.getHusband() != null) ? c.getHusband() : c.getWife();
                heiarchy.getGeneration(3).addCouple(new Couple(child, null));
                return;
            }
            default -> {
            }
        }
    int x = (n==1 || n==2) ? (int)(Math.random() * 3 + 1) : (int)(Math.random() * 2 + 2); 
